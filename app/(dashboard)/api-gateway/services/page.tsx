...
import { Server, Globe, Activity, MoreVertical, Edit, Trash, Loader2, Plus, Search, Filter, ArrowUpRight, Cpu, Layers } from 'lucide-react';
...
export default function GlobalServicesPage() {
  ...
  const filteredServices = services.filter(svc => 
    ...
    svc.gatewayName.toLowerCase().includes(searchQuery.toLowerCase())
  );

  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Premium Header Section - Same Size as Gateway/Targets */}
      <div className="relative overflow-hidden bg-gradient-to-br from-gray-900/60 to-purple-900/10 border border-gray-800/60 rounded-3xl p-8 backdrop-blur-md">
        <div className="absolute top-[-20%] right-[-10%] opacity-10 blur-3xl">
          <Server className="w-96 h-96 text-purple-500" />
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
          <div className="space-y-3">
            ...
            <p className="text-gray-400 max-w-2xl text-sm leading-relaxed font-medium">
              Monitor and orchestrate your entire microservices mesh. View service protocols, load balancing strategies, and health metrics across all deployed gateways.
            </p>
            
            <div className="flex flex-wrap gap-3 pt-2">
              <div className="flex items-center gap-2 bg-gray-950/50 border border-gray-800 px-3 py-1.5 rounded-xl">
                <Cpu className="w-3.5 h-3.5 text-purple-400" />
                <span className="text-[10px] font-bold text-gray-300">Auto-Scaling Ready</span>
              </div>
              <div className="flex items-center gap-2 bg-gray-950/50 border border-gray-800 px-3 py-1.5 rounded-xl">
                <Layers className="w-3.5 h-3.5 text-blue-400" />
                <span className="text-[10px] font-bold text-gray-300">Cross-Gateway Visibility</span>
              </div>
            </div>
          </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64 p-2 bg-gray-900 border-gray-800 text-gray-100 rounded-2xl shadow-2xl backdrop-blur-xl">
              ...
              {gateways.length === 0 ? (
                ...
                gateways.map(gw => (
                  <DropdownMenuItem key={gw.id} onClick={() => router.push(`/api-gateway/${gw.id}/services/new`)} className="flex items-center gap-3 p-3 cursor-pointer rounded-xl hover:bg-blue-500/10 hover:text-blue-400 transition-all group/item">
                    <Globe className="w-4 h-4 text-blue-400" />
                    <div className="flex flex-col">
                      <span className="font-bold text-sm">{gw.name}</span>
                      <span className="text-[10px] text-gray-500 group-hover/item:text-blue-300/60">Global Gateway</span>
                    </div>
                  </DropdownMenuItem>
                ))
              )}
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-gray-950/50 p-4 rounded-2xl border border-gray-800/40">
        <div className="relative flex-1 w-full">
          ...
          <Input 
            placeholder="Search services by name or gateway..." 
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto">
          ...
          <div className="px-4 py-2 bg-gray-900/50 border border-gray-800 rounded-xl hidden lg:block">
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Total Services: </span>
            <span className="text-sm font-bold text-blue-400 ml-1">{services.length}</span>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-950/40 border-b border-gray-800 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                ...
                <th className="px-8 py-5">LB Policy</th>
                <th className="px-8 py-5 text-center">Targets</th>
            ...
            <tbody className="divide-y divide-gray-800/40">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center justify-center max-w-xs mx-auto">
                      ...
                      <h3 className="text-lg font-bold text-gray-200">No services found</h3>
                      <p className="text-gray-500 text-sm mt-2 text-balance">
                        {searchQuery ? `No matches found for "${searchQuery}"` : "Start by adding an upstream service to one of your gateways."}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                filteredServices.map((svc) => (
                  <tr key={svc.id} className="group hover:bg-blue-500/[0.02] transition-all duration-300">
                    <td className="px-8 py-5">
                      <Link href={`/api-gateway/${svc.gateway_id}/services/${svc.id}`} className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-gray-100 group-hover:text-blue-400 transition-colors tracking-tight">
                            {svc.name}
                          </span>
                          <span className="text-[10px] text-gray-500 font-medium">Upstream Mesh Service</span>
                        </div>
                      </Link>
                    </td>
                    <td className="px-8 py-5">
                      <Link href={`/api-gateway/${svc.gateway_id}/services`} className="inline-flex items-center gap-2 text-gray-400 hover:text-blue-400 transition-colors">
                        <Globe className="w-3.5 h-3.5" />
                        <span className="text-xs font-medium">{svc.gatewayName}</span>
                      </Link>
                    </td>
                    ...
                    <td className="px-8 py-5">
                      <span className="text-xs font-mono font-bold text-gray-400 bg-gray-800/40 px-2 py-1 rounded-md border border-gray-800/60">
                        {svc.lb_policy}
                      </span>
                    </td>
                    <td className="px-8 py-5 text-center">
                      <div className="flex justify-center">
                        <span className="inline-flex items-center justify-center min-w-[24px] h-6 px-1.5 rounded-full bg-gray-800 text-blue-400 text-[10px] font-black border border-gray-700 shadow-inner">
                          {svc.targetsCount}
                        </span>
                      </div>
                    </td>
                    <td className="px-8 py-5 text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <button disabled={isDeleting === svc.id} className="p-2 text-gray-500 hover:text-gray-100 hover:bg-gray-800 rounded-xl transition-all disabled:opacity-50">
                            {isDeleting === svc.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <MoreVertical className="w-5 h-5" />}
                          </button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-56 p-1 bg-gray-900 border-gray-800 text-gray-100 rounded-xl shadow-2xl backdrop-blur-xl">
                          <DropdownMenuItem onClick={() => router.push(`/api-gateway/${svc.gateway_id}/services/${svc.id}/edit`)} className="flex items-center gap-2 p-2.5 cursor-pointer rounded-lg hover:bg-blue-500/10 hover:text-blue-400 transition-all">
                            <Edit className="w-4 h-4" />
                            <span className="font-medium text-sm">Modify Settings</span>
                          </DropdownMenuItem>
                          ...
                          >
                            ...
                            <span className="font-medium text-sm">Terminate Service</span>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </td>
