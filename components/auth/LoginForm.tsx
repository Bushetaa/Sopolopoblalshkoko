"use client";

import { useState } from "react";
import { useForm } from "react-hook-form";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { useAuth } from "@/hooks/useAuth";
import { Loader2, ArrowRight } from "lucide-react";
import { apiClient } from "@/lib/api-client";
import { toast } from "@/hooks/use-toast";

const schema = z.object({
  email: z.string().email("Enter a valid email"),
  password: z.string().optional(),
  otp: z.string().optional(),
});

type Values = z.infer<typeof schema>;

export function LoginForm() {
  const { login, signInOTP, verifyOTP, isAuthenticating } = useAuth();
  const [serverError, setServerError] = useState<string | null>(null);
  const [isOtpMode, setIsOtpMode] = useState(false);
  const [isOtpSent, setIsOtpSent] = useState(false);

  const form = useForm<Values>({
    resolver: zodResolver(schema),
    defaultValues: { email: "", password: "", otp: "" },
  });

  const onSubmit = async (values: Values) => {
    setServerError(null);
    try {
      if (isOtpMode) {
        if (isOtpSent) {
          if (!values.otp) {
            form.setError("otp", { message: "Code is required" });
            return;
          }
          await verifyOTP(values.email, values.otp);
        } else {
          await signInOTP(values.email);
          setIsOtpSent(true);
        }
      } else {
        if (!values.password) {
          form.setError("password", { message: "Password is required" });
          return;
        }
        await login(values.email, values.password);
      }
    } catch (error: any) {
      setServerError(error.message || "Failed to sign in");
    }
  };

  const handleResendVerification = async () => {
    try {
      const email = form.getValues("email");
      if (!email) {
        setServerError("Please enter your email first.");
        return;
      }
      await apiClient.post("/auth/user/email/send-verification-email", { email });
      toast({ title: "Email Sent", description: "Verification email resent successfully." });
    } catch (error: any) {
      toast({ title: "Error", description: "Failed to resend verification email.", variant: "destructive" });
    }
  };

  const needsVerification = serverError?.toLowerCase().includes("verify") || serverError?.toLowerCase().includes("unverified");

  return (
    <Form {...form}>
      <form 
        onSubmit={(e) => {
          e.preventDefault();
          form.handleSubmit(onSubmit)(e);
        }} 
        className="space-y-4"
      >
        {serverError && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-500 px-3 py-3 rounded text-sm flex flex-col gap-2">
            <span>{serverError}</span>
            {needsVerification && (
              <Button 
                variant="outline" 
                size="sm" 
                type="button" 
                onClick={handleResendVerification}
                className="w-fit border-red-500/50 hover:bg-red-500/20 text-red-500 mt-1"
              >
                Resend Verification Email
              </Button>
            )}
          </div>
        )}

        {!isOtpMode && (
          <div className="grid grid-cols-2 gap-4">
            <Button 
              variant="outline" 
              type="button"
              disabled={isAuthenticating}
              className="w-full h-11 border-zinc-300 dark:border-zinc-800 hover:bg-[#24292F] hover:text-white hover:border-[#24292F] dark:hover:bg-white dark:hover:text-black dark:hover:border-white transition-all duration-300 group" 
              onClick={() => window.location.href = `${process.env.NEXT_PUBLIC_AUTH_SERVICE_URL}/signin/provider/github?redirectTo=${encodeURIComponent(window.location.origin + '/dashboard')}`}
            >
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
              </svg>
            </Button>
            <Button 
              variant="outline" 
              type="button"
              disabled={isAuthenticating}
              className="w-full h-11 border-zinc-300 dark:border-zinc-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-[#4285F4] dark:hover:border-[#4285F4] transition-all duration-300 group" 
              onClick={() => window.location.href = `${process.env.NEXT_PUBLIC_AUTH_SERVICE_URL}/signin/provider/google?redirectTo=${encodeURIComponent(window.location.origin + '/dashboard')}`}
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                <path d="M1 1h22v22H1z" fill="none" />
              </svg>
            </Button>
          </div>
        )}

        <div className="relative my-4">
          <div className="absolute inset-0 flex items-center">
            <span className="w-full border-t border-zinc-300 dark:border-zinc-800" />
          </div>
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white dark:bg-zinc-950 px-2 text-muted-foreground">
              {isOtpMode ? "Sign in with OTP" : "Or continue with email"}
            </span>
          </div>
        </div>

        <FormField
          control={form.control}
          name="email"
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-zinc-700 dark:text-zinc-300">Email</FormLabel>
              <FormControl>
                <Input
                  type="email"
                  disabled={isAuthenticating || isOtpSent}
                  className="bg-white dark:bg-zinc-900/60 border border-zinc-300 dark:border-zinc-800 text-black dark:text-white disabled:opacity-50"
                  {...field}
                />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        />

        {!isOtpMode && (
          <FormField
            control={form.control}
            name="password"
            render={({ field }) => (
              <FormItem>
                <div className="flex items-center justify-between">
                  <FormLabel className="text-zinc-700 dark:text-zinc-300">Password</FormLabel>
                  <a 
                    href="/forgot-password" 
                    className="text-xs text-primary hover:underline underline-offset-4"
                  >
                    Forgot password?
                  </a>
                </div>
                <FormControl>
                  <Input
                    type="password"
                    disabled={isAuthenticating}
                    className="bg-white dark:bg-zinc-900/60 border border-zinc-300 dark:border-zinc-800 text-black dark:text-white disabled:opacity-50"
                    {...field}
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          />
        )}

        {isOtpMode && isOtpSent && (
          <FormField
            control={form.control}
            name="otp"
            render={({ field }) => (
              <FormItem className="animate-in fade-in duration-300">
                <FormLabel className="text-zinc-700 dark:text-zinc-300">One-Time Passcode</FormLabel>
                <FormControl>
                  <Input
                    type="text"
                    disabled={isAuthenticating}
                    placeholder="Enter the 6-digit code"
                    className="bg-white dark:bg-zinc-900/60 border border-zinc-300 dark:border-zinc-800 text-black dark:text-white disabled:opacity-50"
                    {...field}
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          />
        )}

        <Button 
          type="submit" 
          disabled={isAuthenticating}
          className="w-full bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {isAuthenticating && <Loader2 className="h-4 w-4 animate-spin" />}
          {isAuthenticating ? "Please wait..." : isOtpMode ? (isOtpSent ? "Verify Code" : "Send Code") : "Sign in"}
        </Button>

        <div className="pt-2 text-center">
          <button
            type="button"
            onClick={() => {
              setIsOtpMode(!isOtpMode);
              setIsOtpSent(false);
              setServerError(null);
              form.setValue("otp", "");
            }}
            className="text-sm text-muted-foreground hover:text-foreground transition-colors underline decoration-dotted underline-offset-4"
          >
            {isOtpMode ? "Sign in with password instead" : "Sign in with a one-time passcode (OTP)"}
          </button>
        </div>
      </form>
    </Form>
  );
}
